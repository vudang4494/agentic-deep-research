# [ARCHIVED] Plan v3 — thiết kế & khung đánh giá gốc (2026-06-04 → 06-07)

> ⚠️ **TÀI LIỆU LỊCH SỬ — ĐÃ SUPERSEDE. ĐỪNG TRÍCH SỐ Ở ĐÂY LÀM FACT.**
> Đây là bản thiết kế + khung eval của v3 lúc mới dựng, tách khỏi `docs/plan.md` ngày 2026-07-24 để plan.md quay về đúng vai "roadmap việc còn phải làm".
>
> **Những chỗ đã biết là SAI so với code hôm nay** (giữ nguyên văn để không viết lại lịch sử):
> - `P0a ... < 0.60` — ngưỡng thật ≈0.40 (`ev_threshold`).
> - Công thức P0c `max(0.1, 1 - seen_count/50 * 0.8)` — thật là `max(0.05, (1 − seen/max_seen)²)`.
> - Mọi kết luận dạng *"grounding g=1.000 → PASS"* — grounding hồi đó **degenerate** (HHEM mất weight-tying, trả hằng số), và nay là **advisory, đã bỏ khỏi gate**. Đừng dùng làm bằng chứng chất lượng.
> - `P1/P2/P3/P4` trong file này là **hệ đánh số CŨ** (prerequisite fixes), **KHÔNG liên quan** tới `P1-1..P1-5`/`P2` trong `docs/plan.md` hiện hành.
> - Bảng metric & "Experiment A/B/C/D", "Current Product Reality (2026-06-07)" là ảnh chụp tháng 6, không phải trạng thái hiện tại.
>
> Trạng thái hôm nay → `memory/short-memory.md` · Lịch sử → `memory/long-memory.md` · Ngưỡng → `docs/RULES.md` · Doctrine → `CLAUDE.md §2`.
