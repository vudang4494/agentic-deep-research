...
⚠️ **Bẫy hằng số trùng tên:** `AUTO_SUPPORT_COS` tồn tại ở CẢ `config.py` (legacy/v1) lẫn `verify.py` (bản đang chạy, giá trị KHÁC) — đường verify dùng bản của `verify.py`, sửa `config.py` KHÔNG đổi hành vi G2. Tương tự `config.py MIN_GROUNDING` là hằng số **chết** (grounding đã ra khỏi gate). **Trước khi đổi bất kỳ ngưỡng nào: `grep -rn "<TÊN_HẰNG>" research/` để biết bản nào thắng.**
