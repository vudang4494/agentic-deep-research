...
def _atomic_write_text(path, text):
    """Write text atomically: dump to a sibling temp file, then os.replace (atomic on POSIX).
    The resume-critical state/profile JSON is rewritten after every section; a plain write_text
    truncates-then-writes, so a kill (`pkill` is the documented stop) or crash mid-write leaves a
    half-written file that the next resume's json.loads aborts on -- stranding every completed
    section. os.replace makes the swap atomic, so a resume never sees a truncated JSON."""
    import os
...
def run_v3(topic, out_name=None, n_chapters=None, sections_per_chapter=None,
           ...
           target_section=None, force_regen=False):
    ...
    if topic_path.exists():
        ...
        if new_ids and not new_ids.issubset(existing_ids):
            ...
            _atomic_write_text(topic_path, json.dumps(asdict(topic_profile), indent=2, ensure_ascii=False))
        else:
    ...
    if outline_path.exists():
        ...
        data.pop("_raw", None)  # don't reload the bulky raw LLM string
        # Filter to current dataclass fields (mirror the topic-profile resume path) so an older
        # run whose JSON carries a since-renamed/removed key doesn't crash OutlineProfile(**data).
        _op_fields = {f.name for f in OutlineProfile.__dataclass_fields__.values()}
        outline = OutlineProfile(**{k: v for k, v in data.items() if k in _op_fields})
    else:
        ...
        _atomic_write_text(outline_path, json.dumps(asdict(outline), indent=2, ensure_ascii=False))
    ...
    if target_section and force_regen and sections:  # --force-regen gates the destructive clear
        matched_keys = []
        t_low = target_section.lower().strip()
        for k, v in list(sections.items()):
            sec_title = (v.get("title") or "").lower()
            if t_low in k.lower() or t_low in sec_title or t_low in f"section {k}".lower():
                matched_keys.append(k)
        if matched_keys:
            for k in matched_keys:
                popped = sections.pop(k, None)
                if popped:
                    total_w -= len((popped.get("content") or "").split())
            total_w = max(0, total_w)
            print(f"  [TARGET-REGEN] Cleared {len(matched_keys)} section(s) for target '{target_section}': {', '.join(matched_keys)}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--target-section", default=None,
                   help="Target section title or key (e.g. '1.2' or 'Attention') to selectively regenerate")
    p.add_argument("--force-regen", action="store_true",
                   help="Force regenerate the targeted section")
    args = p.parse_args()
