...
- **[07-31/08-01] `gen_ai_900p`: 665 trang / 334 section / 18,5h — baseline mới.** block **1,5%** (run trước 23,3%), topic **0,902**, expl **0,631** (cao nhất), cite_prec 0,356. Block-rate sụp gần như chắc chắn nhờ `arxiv_by_id` fallback (PR#41). Cả 3 fix trong phiên đều nhìn thấy trên bản in: **0** URL redirect (PR#43), **0** cặp section trùng (PR#44 làm `relate/dedup` chạy lại được → 299 `depends_on` edges, 73 near-dup phân hoá). **Phát hiện định lượng:** `cite_precision` **bất động** xuyên 3 run (0,352–0,385) trong khi mọi thứ khác dao động mạnh → bằng chứng số cho "hình phạt tổng hợp là nội tại".
- **[07-31] Lỗi batch embed — hỏng mà KHÔNG báo, và tự tắt đúng lúc cần nhất.** `/api/embed` từ chối theo **số lượng text** (128 ok / 192 fail; 586 KB trong 20 text vẫn OK → không phải giới hạn dung lượng). `embed()` trả `[]` khi lỗi và mọi caller hiểu `[]` là "degrade cho êm", nên vượt trần **không ném lỗi**: outline 35 chương mất trắng bước gỡ-trùng-xuyên-chương (`dropped 0 cross-chapter dup`, 7 cặp trùng sống sót). Vì batch tỉ lệ với kích thước sách, bảo vệ biến mất đúng lúc sách đủ lớn để cần. Vá: chunk 64 + retry/chunk + **không bao giờ trả kết quả một phần**. Enforce bởi `verify_all` **check K** (đã test negative: bắt được cả `_MAX_BATCH` vượt trần lẫn bị xoá). PR#44.
- **[07-31] Trần quy mô outline đo được.** Xin `--n-chapters 60 × 10 = 600` → nhận **39 chương / 334 section**, trong đó **8 chương chỉ là khuôn** `<tên chương>: Core Mechanisms/Design and Trade-offs/Practical Methods` — vì Discovery chỉ tìm được ~49 nguồn. `enforce_outline_structure` thu gọn 56 section matrix (chặn phần lớn, không hết). → **Đòn bẩy cho sách dài hơn là NGUỒN, không phải `--n-chapters`.**
- **[07-30] Thí nghiệm thất bại thứ 4: lọc canonical theo section — chết TRƯỚC khi ship.** Triệu chứng thật (section red-teaming trích "Attention Is All You Need"; canonical miễn prefilter+P0c nên **chiếm** 1 trong ~3 slot bằng chứng). Quy tắc tỉ lệ 0,82 phân tách sạch khi hiệu chuẩn trên 4 paper toàn sách, nhưng replay trên 86 section thật thì **4/5 paper bị bỏ là cái đang được TRÍCH** và **3 section rơi dưới `_evidence_adequate`** (7.1: 3→1 nguồn). Nguyên nhân: tuned trên 4 ứng viên nhưng thực tế chỉ 2–3 canonical sống sót vào một section. Khác 3 lần trước: **cổng verify là replay trên run thật, viết TRƯỚC khi ship** — unit test không nhìn thấy loại hỏng này.

- **[07-28/29] Ba kết quả ÂM về "hình phạt dành cho tổng hợp" — giá trị nhất của phiên.** Chẩn đoán từ đọc code: G2 chấm MỘT CÂU cho mỗi `[N]`, `AUTO_SUPPORT_COS=0.90` cho paraphrase sát 1.0 miễn phí, câu ghép 2 nguồn bị chấm rời từng nguồn → 0.0; revise-loop còn ra lệnh *"restate the claim to match what its source actually says"*. Thử **CitationAgent** (tách trích dẫn khỏi writer, như hệ multi-agent research của Anthropic) → 95 lần gọi, ΔG2 **+0.028** = nhiễu. Thử **G2 theo claim** (hợp nguồn + miễn câu suy diễn) → chấm lại 66 section: **0.352→0.323**, siết chứ không nới. **Cả hai đã revert, `verify.py` nguyên trạng.** Kết luận: hình phạt tổng hợp **nội tại** ở việc chấm prose tổng hợp với excerpt truy xuất, không phải lỗi đơn vị chấm. → ghi trong khối `TRIED AND REVERTED`.
- **[07-28] Canonical protection hỏng âm thầm, hai đường.** `arxiv_by_id` **không có fallback** (arxiv timeout 7s/20s, HTTP 429 ở 40s → 0 paper) trong khi `arxiv_search` thì có; và ID canonical được so bằng **hai ngữ nghĩa khác nhau** ở hai nơi (P0b chuẩn hoá, chỗ khác so tuyệt đối). Canonical miễn prefilter+P0c nên mất chúng **không ném lỗi nào**. Vá: timeout 25s + retry + Tavily-theo-ID (chỉ nhận đúng ID — canonical *sai* hại hơn *thiếu*) + `notes.normalize_source_id` single-source. PR #41.
- **[07-28] Đánh đổi dạy-vs-quy-nguồn đo được lần đầu.** Canonical nhận đoạn liền mạch dày công thức 2200w thay cửa sổ 550w bám claim: `explanation_depth` 0.503→0.599, `cite_precision` 0.385→0.345; section 1.3 đi từ `ok` sang `degraded` **vì nó dạy nhiều hơn**. Giữ (dạy là mục tiêu sản phẩm; `degraded` vẫn vào sách). Số ghi tại hằng số `_CANONICAL_EXCERPT_WORDS`.
- **[07-28] Stage-F `mdfix`** — 4 lỗi markdown khiến sách *in ra* tệ hơn nội dung thật: list dính paragraph bị pandoc bẹp thành văn xuôi (9/15 section, giết đúng phần dạy mà `explain.py` vẫn tính điểm), ref block đánh số lại làm `[3]` trong prose lệch mục in (8/15 = **vỡ attribution**), `[[N]]`, `[N]` lạc sau `$$`. Formatting-only + idempotent → assembler và renderer dùng chung. PR #40.
- **[07-28] Xoá `research/planner.py`** (414 dòng): v3 chưa bao giờ import mà `CLAUDE.md` liệt kê là load-bearing. Bài học: doc nói một module là load-bearing **không** thay được `grep`.
- **[07-28] Deep-research workflow: chi phí sai gấp bội so với giá trị.** 105 agent · 4.1M token · bước synthesize hỏng vì chạm limit; resume lại đẻ thêm ~98 agent (cache key nhúng nguyên văn claim → đổi upstream là miss toàn bộ pha verify). Chỉ **4/13 claim** thực sự dùng được, và chẩn đoán cốt lõi thì đến từ `grep` **miễn phí**. → Quy tắc: nêu số agent + token ước lượng TRƯỚC khi fan-out; với repo này, đọc code + 1 run local là công cụ đúng.
...
- **PR#26:** thêm `brave` vào `PROVIDERS_DEFAULT` — thay thế FREE cho Tavily lúc đó billing-dead (HTTP 402) — **đã hết đúng từ 2026-07, Tavily sống lại với key mới**. Thiếu key = no-op an toàn.
