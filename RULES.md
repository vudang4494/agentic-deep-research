...
| Embed model | **`bge-m3:latest`** (notes/verify/embeddings) | — | (config.py ghi `nomic-embed-text` = DRIFT) |

⚠️ `config.py MIN_GROUNDING=0.55` và `EMBED_MODEL=nomic-embed-text` KHÔNG phải giá trị vận hành — `deep_investigate.investigate_section()` default `min_grounding=0.70` và code thật dùng bge-m3. Reconcile trước khi tin tuning.
