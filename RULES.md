...
> ✅ **P0 ĐÃ APPLY (2026-06-22) — trạng thái GATE hiện tại (đã sửa so với mô tả "INERT" pre-P0):**
> - Grounding **bỏ khỏi gate** → log-only/advisory. `base_ok` cũ thay bằng `gate_ok = n_cites>0 AND topic≥0.50 AND cross-ref` (`deep_investigate.py:752`).
> - G2 `verify_section` **GIỜ CHẠY** khi `n_cites>0 AND topic_ok` (bất kể grounding) → `cite_precision` **đo thật** (RLHF: 0.30–0.41, ≠ default 1.0).
> - StageE chuyển **sau-loop**, chặn topic-drift theo **best-topic**; best-round chọn **topic-first** (`:716,808`).
> - P0c aliasing **fixed** (`:304`) → `run_seen_counts` populate (0→23).
> ⚠️ **CÒN LẠI (P0-2b):** gemma cite-judge **strict-match** ("direct match only", `verify.py`) → cite_precision floor ~0.3–0.4 < `min_cite_precision=0.45` → **clean-accept vẫn = 0** (mọi section `degraded`). G2 hết là fake-1.0 NHƯNG chưa thành gate dùng được tới khi soften judge / recalibrate.
> → **Gate cứng SỐNG vẫn = P0a domain-evidence (~0.40, PRE-writer)** + StageD word-count. Chi tiết: `plan.md` §Upgrade P0/P0-2b.
...
| Citation precision (G2) | **CHẠY** (P0): ≥ **0.45** để clean-accept; đo thật per-[N]. ⚠️ judge strict-match → floor ~0.3-0.4 → clean-accept=0, cần **P0-2b** soften/recalibrate | `deep_investigate.py:739-748` | discriminate, không floor |
...
| **D** Verify | clean-accept = topic≥0.50 (G4) + n_cites>0 + cross-ref + cite_precision≥0.45 (G2); grounding log-only | **P0 applied:** G2 GIỜ CHẠY (cite_precision đo thật), grounding bỏ khỏi gate, StageE chặn best-topic<0.50 sau-loop. ⚠️ cite-judge strict-match → cite_precision floor ~0.3-0.4 < 0.45 → clean-accept=0 (cần **P0-2b**). Gate cứng SỐNG vẫn = P0a (Gate B). | `deep_investigate.py:739-808`, `verify.py` |
...
**LIVE (v3, post-P0 2026-06-22):** `grounding_score` (HHEM, **G3 log-only/advisory** — đã bỏ khỏi gate) + `topic_relevance_check` (G4 — **ENFORCED**: điều kiện `gate_ok` + StageE best-topic) + `verify_section` (G2 cite_precision — **GIỜ CHẠY**, đo thật; clean-accept cần ≥0.45 — ⚠️ judge strict-match floor ~0.3-0.4, cần **P0-2b**) + `verify_cross_references_v2` (regex đếm). **Gate cứng SỐNG = P0a (pre-writer ~0.40) + StageD word-count.** **LEGACY-only** (`deep_research.py`/scripts/eval — đừng sửa như live): `verify_section_v2`, `crag_decision`, `strip_refine`, `scrub_unsupported_citations`.
...
- G1 **P0a** domain ≈0.40 HARD ✅ (pre-writer, gate cứng SỐNG) · G3 **grounding = log-only/advisory** (P0: bỏ khỏi gate) · G4 **topic** ✅ ENFORCED (gate_ok + StageE best-topic) · G2 **citation integrity** ✅ CHẠY (P0; cite_precision đo thật, ≥0.45 để clean-accept) — ⚠️ judge strict floor ~0.3-0.4 → clean-accept=0, cần **P0-2b** soften/recalibrate · G5a cross-ref `:630` · G5b numeric-ref hint (soft). **→ P0 done (G2 sống, P0c fixed, grounding advisory); P0-2b = làm cite-judge discriminate (`plan.md`).**
