...
> ⚠️ **THỰC TRẠNG GATE (verified trên 4-topic, 390 section — ĐỌC TRƯỚC khi tin các row "accept"):**
> Per-source-max grounding **KHÔNG BAO GIỜ chạm 0.70** (max thực 0.458; mọi section `quality='degraded'`, 0 "ok").
> Vì `base_ok` yêu cầu grounding≥0.70, **`base_ok` LUÔN false** → (a) clean-accept không bao giờ fire;
> (b) `verify_section` (G2) nằm trong `if base_ok:` → **KHÔNG BAO GIỜ chạy** → `cite_precision=1.0` là **DEFAULT init**
> (BAER parse nhầm từ dòng retry-hint `cite_prec=1.000`, KHÔNG phải đo thật); (c) StageE topic-block (cũng cần
> grounding≥0.70) **không bao giờ fire**.
> → **Gate cứng SỐNG duy nhất** định hình accept/block = **P0a domain-evidence (~0.40, PRE-writer)** + StageD
> word-count 120 + empty-pool. Mọi tín hiệu **POST-writer (G3 grounding / G4 topic / G2 citation) chỉ LOG, KHÔNG
> enforce.** Đây là **bug thiết kế** → fix ở `plan.md` §Upgrade P0.
...
| Topic relevance (G4) | **LOG-only** post-writer — StageE cần grounding≥0.70 nên KHÔNG enforce; tương quan P0a → topic≈accept (vòng tròn) | `deep_investigate.py:683,751` | enforce thật |
| Grounding (HHEM/G3) | **INERT** — conjunct của `base_ok` ở 0.70 nhưng per-source-max max 0.458 → base_ok luôn false → mọi section `'degraded'`; strict-NLI ~0.05–0.10. KHÔNG phải metric | `deep_investigate.py:227,729,850` | re-baseline / bỏ khỏi base_ok |
| StageE hard block | **KHÔNG BAO GIỜ fire** (cần grounding≥0.70) | `deep_investigate.py:751` | (cần re-baseline) |
| Citation | n_cites > 0 (trong base_ok, nhưng base_ok luôn false → không gate) | `deep_investigate.py:729` | diversity ≥ 10 unique |
| Citation precision (G2) | **INERT** — `verify_section` nằm trong `if base_ok` (luôn false) → KHÔNG BAO GIỜ chạy; `cite_precision=1.0` là DEFAULT, KHÔNG đo | `deep_investigate.py:230,737,745` | đo thật per-[N] |
...
| **D** Verify | (định nghĩa: topic 0.50 + n_cites + cross-ref + cite_precision 0.45 + grounding) | ⚠️ **THỰC TẾ INERT** — accept-clean + StageE đều gate qua `base_ok` cần grounding≥0.70, mà per-source-max grounding max 0.458 → **base_ok LUÔN false** → KHÔNG nhánh verify nào hard-block; `verify_section` (G2) nằm trong `if base_ok` → **không bao giờ chạy** (`cite_precision=1.0` = DEFAULT). HHEM re-tie phân biệt cặp NLI sạch nhưng strict-NLI ~0.05–0.10 trên prose synthesized. **Gate cứng THẬT = P0a (Gate B, pre-writer), KHÔNG phải Gate D.** Fix → `plan.md` §Upgrade P0. | `deep_investigate.py`, `verify.py`, `faithfulness.py` |
...
**LIVE (v3, trong `investigate_section`) — nhưng PHẢI phân biệt cái nào THẬT enforce:** `grounding_score` (HHEM, **G3 INERT** — log-only; base_ok cần ≥0.70 mà per-source-max max 0.458) + `topic_relevance_check` (G4 — **chạy nhưng KHÔNG enforce**: StageE cần grounding≥0.70 nên không fire) + `verify_section` (G2 cite_precision — **KHÔNG BAO GIỜ chạy**, kẹt sau `if base_ok` luôn false; `cite_precision=1.0` = default) + `verify_cross_references_v2` (regex đếm). **⚠️ Chỉ P0a (Gate B, pre-writer) thật sự hard-block; mọi verify post-writer là log-only.** **LEGACY-only** (`deep_research.py`/scripts/eval — đừng sửa như live): `verify_section_v2`, `crag_decision`, `strip_refine`, `scrub_unsupported_citations`.
...
- G1 **P0a** domain ≈0.40 HARD ✅ (**gate cứng SỐNG duy nhất**, pre-writer) · G3 **grounding = INERT** (log-only; base_ok cần ≥0.70 mà per-source-max max 0.458 → không bao giờ pass) · G4 **topic judge** ⚠️ (`topic_relevance_check` blend gemma local **chạy nhưng KHÔNG enforce** — StageE không fire) · G2 **citation integrity** ❌ (`verify_section` per-`[N]` **KHÔNG BAO GIỜ chạy**, kẹt sau `if base_ok` luôn false; `cite_precision=1.0`=default) · G5a cross-ref `:630` (đếm, trong base_ok) · G5b numeric-ref hint (soft). **→ Toàn bộ verify post-writer hiện là log-only; fix decouple G2 + re-baseline grounding ở `plan.md` §Upgrade P0.**
...
2. Grounding pass ≠ success (StageE thiết kế để block g-pass + topic-fail — NHƯNG hiện **không bao giờ fire** vì grounding không chạm 0.70; cần re-baseline, xem §Upgrade P0).
